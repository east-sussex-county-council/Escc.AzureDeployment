...
:: Now that we have the latest scripts, update the deployment script

:: If we already have a script, pull from Azure to make sure the deployment script is in sync.

if exist KuduDeploy.cmd (
  echo.
  echo ------------------------------------------------------
...
echo :: To update the content of this file, modify DeployOnAzure.cmd >> Kudu\KuduDeploy.cmd
echo :: then run ..\Escc.AzureDeployment\UpdateDeploymentScript.cmd >> Kudu\KuduDeploy.cmd
echo. >> Kudu\KuduDeploy.cmd
echo @echo ------------------------------------------------------ >> Kudu\KuduDeploy.cmd
echo @echo Running KuduDeploy.cmd generated at %date% %time% >> Kudu\KuduDeploy.cmd
echo @echo ------------------------------------------------------ >> Kudu\KuduDeploy.cmd
echo @echo. >> Kudu\KuduDeploy.cmd
type %ESCC_DEPLOYMENT_SCRIPTS%Kudu\DeployPart1.cmd %ESCC_DEPLOYMENT_START_PATH%\DeployOnAzure.cmd %ESCC_DEPLOYMENT_SCRIPTS%Kudu\DeployPart3.cmd >> Kudu\KuduDeploy.cmd
...
echo.
echo Make sure you have a git remote called 'azure' with the URL of your Azure 
echo website's git repository. You can find the URL on the Deployments 
echo page for your website in the Azure portal. Then push this repository to 
echo the 'azure' remote.
echo -------------------------------------------------------------------------
echo.
